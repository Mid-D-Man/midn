...
mod tests {
    ...
    fn h16(s: &str) -> [u8; 16] { hbytes::<16>(s) }
    fn h8(s: &str)  -> [u8;  8] { hbytes::< 8>(s) }
    fn h6(s: &str)  -> [u8;  6] { hbytes::< 6>(s) }
    ...
    fn verify_res_mismatch_returns_false() {
        let xres    = h8("a54211d5e3ba50bf");
        let bad_res = h8("a54211d5e3ba50be");
        assert!(!MilenageContext::verify_res(&xres, &bad_res));
    }

    #[test]
    fn verify_res_all_zeros_vs_nonzero() {
        assert!(!MilenageContext::verify_res(&[0u8; 8], &h8("a54211d5e3ba50bf")));
    }
