...
//!
//! `amf` was a stub (`registration.rs` — a comment-only placeholder) until
//! this increment. Now implements the 5G Registration procedure's Phase A
//! (no PDU session / TEID / UPF interaction — see `amf::registration`
//! module doc for exactly what that means and why). `pub mod amf;` was
//! deliberately left out of this file until there was real content behind
//! it — see that decision recorded in project history; no longer applies.
//!
