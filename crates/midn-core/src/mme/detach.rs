...
//! the NAS request, optionally ack it, and ask the eNodeB to release the
//! S1/radio context. There is exactly one teardown path in the system, no
//! matter which side initiates it.

use midn_proto::nas::{decode_nas, encode_detach_accept, NasPdu};
...
mod tests {
    ...
    fn normal_detach_sends_accept_then_release_command() {
        let world   = world_with_context(7);
        let nas_pdu = encode_detach_request(1, false, 0, &[0; 10]);
        let out     = handle_detach_request(&world, 1, 7, &nas_pdu);
    ...
    fn detach_for_unknown_entity_is_noop() {
        let world   = World::new(); // no context inserted
        let nas_pdu = encode_detach_request(1, false, 0, &[0; 10]);
        let out     = handle_detach_request(&world, 1, 999, &nas_pdu);
        assert!(out.is_empty());
    }
    ...
    fn bad_nas_pdu_is_noop() {
        let world = world_with_context(7);
        let out   = handle_detach_request(&world, 1, 7, &[0xFF, 0xFF]);
        assert!(out.is_empty());
    }
}
